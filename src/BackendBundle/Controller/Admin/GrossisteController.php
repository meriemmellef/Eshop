 ...
 */
class GrossisteController extends BaseAdminController
{
    ...
    public function preUpdateGrossisteEntity($user)
    {
    ...
    {
        $user->addRole("ROLE_GROSSISTE");
        $bytes = openssl_random_pseudo_bytes(4);
        $generatedPassword = bin2hex($bytes);
        $user->setPlainPassword($generatedPassword);
        $password = $this->get('security.password_encoder')->encodePassword($user, $user->getPlainPassword());
        $user->setPassword($password);
        $this->sendMail($user->getEmail(), "'Total B2B'", "admin/emails/notify_gestionnaire.html.twig", $user);
        $this->get('fos_user.user_manager')->updateUser($user, false);
    }
    ...
    public function affectationAction()
    {
        ...
        $data_affectation = $this->getDoctrine()->getRepository('WebBundle:Utilisateurs')->findListAffecation();

        $count_users = $this->getDoctrine()->getRepository('WebBundle:Region')->CountGrossisteByDelegation();

        return $this->render('BackendBundle:Admin/utlisateur:affectation.html.twig', array('id' => $id, 'data' => $data_affectation, 'count_users' => $count_users));
    }
    public function deleteAction()
    {
        $id = $this->request->query->get('id');
        $grossiste = $this->getDoctrine()->getRepository('WebBundle:Utilisateurs')->find(array('id'=>$id));

        $grcmd = $this->getDoctrine()->getRepository('WebBundle:Commandes')->findByGrossiste(array('grossite'=>$id));
        if (count($grcmd)==0){
            $this->get('fos_user.user_manager')->deleteUser($grossiste, false);
